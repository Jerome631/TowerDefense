...
public class BarScript : MonoBehaviour
{
    ...
    {
        //Lerp is linear interpolation which is basically avg. It can used for values that keep changing over time.
        if (fillAmount != content.fillAmount) //If we have a new fill amount then we know that we need to update the bar
        {
            ...
            if (lerpColors) //If we need to linearly interpolate between two colors
            {   
